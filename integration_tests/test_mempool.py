...
def elysium_mempool(tmp_path_factory):
    path = tmp_path_factory.mktemp("elysium-mempool")
...
@pytest.mark.flaky
def test_mempool(elysium_mempool):
    w3: Web3 = elysium_mempool.w3
    filter = w3.eth.filter("pending")
    assert filter.get_new_entries() == []

    cli = elysium_mempool.cosmos_cli(0)
    # test contract
