...
def test_minimal_gas_price(elysium):
    w3 = elysium.w3
...
def test_native_call(elysium):
    """
    test contract native call on elysium network
...
def test_statesync(elysium):
    # elysium fixture
    # Load elysium-devnet.yaml
    # Spawn pystarport with the yaml, port 26650
    # (multiple nodes will be created based on `validators`)
    # Return a Elysium object (Defined in network.py)
    w3 = elysium.w3
    ...
    wait_for_block(
        elysium.cosmos_cli(0),
        int(elysium.cosmos_cli(0).status()["SyncInfo"]["latest_block_height"]) + 5,
    )
    ...
    # We can not use the elysium fixture to do statesync, since they are full nodes.
    # We can only create a new node with statesync config
    data = Path(elysium.base_dir).parent  # Same data dir as elysium fixture
    chain_id = elysium.config["chain_id"]  # Same chain_id as elysium fixture
    cmd = "elysiumd"
    ...
    wait_for_block(
        ...
        int(elysium.cosmos_cli(0).status()["SyncInfo"]["latest_block_height"]) + 1,
    )
...
def test_failed_transfer_tx(elysium):
    """
    ...
    # test the elysium_getTransactionReceiptsByBlock api
...
def test_tx_inclusion(elysium, max_gas_wanted):
    """
    ...
    modify_command_in_supervisor_config(
        elysium.base_dir / "tasks.ini",
        lambda cmd: fn(cmd),
    )
    elysium.supervisorctl("update")
    wait_for_port(ports.evmrpc_port(elysium.base_port(0)))
...
def test_replay_protection(elysium):
    w3 = elysium.w3
