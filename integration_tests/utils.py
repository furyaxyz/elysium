...
TEST_CONTRACTS = {
    ...
    "ElyBridge": "ElyBridge.sol",
    "ElysiumGravityCancellation": "ElysiumGravityCancellation.sol",
    "TestFRC20": "TestFRC20.sol",
    "TestFRC20Proxy": "TestFRC20Proxy.sol",
...
CONTRACTS = {
    "ModuleFRC20": Path(__file__).parent.parent
    / "x/elysium/types/contracts/ModuleFRC20.json",
    "ModuleFRC21": Path(__file__).parent.parent
    / "x/elysium/types/contracts/ModuleFRC21.json",
    "ModuleFRC20Proxy": Path(__file__).parent.parent
...
def eth_to_bech32(addr, prefix=ELYSIUM_ADDRESS_PREFIX):
...
def elysium_address_from_mnemonics(mnemonics, prefix=ELYSIUM_ADDRESS_PREFIX):
    "return elysium address from mnemonics"
...
def send_to_cosmos(gravity_contract, token_contract, w3, recipient, amount, key=None):
    """
    do approve and sendToElysium on ethereum side
    """
    acct = Account.from_key(key)
    ...
    return send_transaction(
        w3,
        gravity_contract.functions.sendToElysium(
...
def build_batch_tx(w3, cli, txs, key=KEYS["validator"]):
    ...
    return {
        ...
        "auth_info": {
            "signer_infos": [],
            "fee": {
                "amount": [{"denom": "basetely", "amount": str(fee)}],
...
def get_receipts_by_block(w3, blk):
    if isinstance(blk, int):
        blk = hex(blk)
    rsp = w3.provider.make_request("elysium_getTransactionReceiptsByBlock", [blk])
    if "error" not in rsp:
        rsp["result"] = [
...
def multiple_send_to_cosmos(gcontract, tcontract, w3, recipient, amount, keys):
    ...
    for key_from in keys:
        ...
        # generate the tx
        tx = gcontract.functions.sendToElysium(
...
def setup_token_mapping(elysium, name, symbol):
    # deploy contract
    w3 = elysium.w3
    contract = deploy_contract(w3, CONTRACTS[name])

    # setup the contract mapping
    elysium_cli = elysium.cosmos_cli()

    print("contract", contract.address)
    denom = f"elysium{contract.address}"
    ...
    rsp = elysium_cli.query_denom_by_contract(contract.address)
