...
from .network import Chainmain, Elysium, Hermes, setup_custom_elysium
...
def prepare_network(tmp_path, file, incentivized=True, start_relay=True):
    ...
    gen = setup_custom_elysium(tmp_path, 26700, Path(__file__).parent / file)
    elysium = next(gen)
    chainmain = Chainmain(elysium.base_dir.parent / "chainmain-1")
    hermes = Hermes(elysium.base_dir.parent / "relayer.toml")
    # wait for grpc ready
    wait_for_port(ports.grpc_port(chainmain.base_port(0)))  # chainmain grpc
    wait_for_port(ports.grpc_port(elysium.base_port(0)))  # elysium grpc
    ...
    if incentivized:
        ...
        rsp = dst_chain.register_counterparty_payee(
            ...
            from_="relayer",
            fees="100000000baseely",
        )
        assert rsp["code"] == 0, rsp["raw_log"]
    if start_relay:
        elysium.supervisorctl("start", "relayer-demo")
        wait_for_port(hermes.port)
    yield IBCNetwork(elysium, chainmain, hermes, incentivized)
...
def hermes_transfer(ibc):
    assert_ready(ibc)
    # chainmain-1 -> elysium_777-1
    my_ibc0 = "chainmain-1"
    my_ibc1 = "elysium_777-1"
