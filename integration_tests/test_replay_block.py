...
def test_replay_block(custom_elysium):
    w3: web3.Web3 = custom_elysium.w3
    ...
    rsp = w3.provider.make_request(
        "elysium_replayBlock", [hex(receipt1.blockNumber), False]
    )
    ...
    rsp = w3.provider.make_request(
        "elysium_replayBlock", [hex(receipt1.blockNumber), True]
    )
    assert "error" not in rsp, rsp["error"]
    assert 2 == len(rsp["result"])
    replay_receipts = [AttributeDict(receipt_formatter(item)) for item in rsp["result"]]
