...
def test_subscribe_basic(elysium: Elysium):
    """
    ...
    wait_for_port(ports.evmrpc_ws_port(elysium.base_port(0)))
    ...
    async def async_test():
        async with websockets.connect(elysium.w3_ws_endpoint()) as ws:
            c = Client(ws)
            ...
            contract = deploy_contract(elysium.w3, CONTRACTS["TestERC20A"])
            address = contract.address
            await asyncio.gather(*[transfer_test(c, elysium.w3, contract, address)])
            contract = deploy_contract(elysium.w3, CONTRACTS["TestMessageCall"])
            inner = contract.caller.inner()
            begin = time.time()
            await asyncio.gather(*[logs_test(c, elysium.w3, contract, inner)])
