...
from .network import Elysium
from .utils import ADDRS, send_transaction, wait_for_port


def test_versiondb_migration(elysium: Elysium):
    """
    ...
    print(elysium.supervisorctl("stop", "all"))
    cli0 = elysium.cosmos_cli(i=0)
    cli1 = elysium.cosmos_cli(i=1)

    changeset_dir = tempfile.mkdtemp(dir=elysium.base_dir)
    print("dump to:", changeset_dir)
    print(cli1.changeset_dump(changeset_dir))
    snapshot_dir = tempfile.mkdtemp(dir=elysium.base_dir)
    ...
    sst_dir = tempfile.mkdtemp(dir=elysium.base_dir)
    ...
    print(elysium.supervisorctl("start", "elysium_777-1-node0", "elysium_777-1-node1"))
    wait_for_port(ports.evmrpc_port(elysium.base_port(0)))
    wait_for_port(ports.evmrpc_port(elysium.base_port(1)))
