...
def prepare_gravity(custom_elysium, custom_geth):
    """
    - set-delegator-keys
    ...
    w3 = custom_geth.w3
    # set-delegate-keys
    for i, val in enumerate(custom_elysium.config["validators"]):
        # generate gorc config file
        gorc_config_path = custom_elysium.base_dir / f"node{i}/gorc.toml"
        grpc_port = ports.grpc_port(val["base_port"])
        metrics_port = 3000 + i
        gorc_config_path.write_text(
            dump_toml(
                gorc_config(
                    custom_elysium.base_dir / f"node{i}/orchestrator_keystore",
        ...
        gorc.add_eth_key("elysium")  # elysium and eth key derivation are the same
        ...
        cli = custom_elysium.cosmos_cli(i)
    ...
    for i, val in enumerate(custom_elysium.config["validators"]):
        ...
        programs[f"program:{chain_id}-orchestrator{i}"] = {
            "command": (
                ...
                "--cosmos-key elysium --ethereum-key eth --mode AlwaysRelay"
            ),
    ...
    add_ini_sections(custom_elysium.base_dir / "tasks.ini", programs)
    custom_elysium.supervisorctl("update")

    yield GravityBridge(custom_elysium, w3, contract)
...
def setup_cosmos_erc20_contract(cluster, denom, symbol):
    ...
    elysium_cli = cluster.elysium.cosmos_cli()
    wait_for_new_blocks(elysium_cli, 30)
    # Check mapping is done on cluster side
    cosmos_erc20 = elysium_cli.query_gravity_contract_by_denom(denom)
