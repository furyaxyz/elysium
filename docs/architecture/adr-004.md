# ADR 004: Tokens conversion in Elysium
...
This ADR documents the "auto" token conversion mechanism implemented in Elysium which aim to improve user-experience by allowing tokens to be converted to a single type (erc20) while in the underlying chain, multiple type of tokens coexist.
...
This ADR describes a mechanism to convert seamlessly native tokens to evm tokens (erc20) out of an IBC transfer. In a more general concept, any kind of native tokens can be converted to an evm tokens (and vice versa) using elysium native transactions. 
...
- Implement in x/elysium module two cosmos transactions : `convert-vouchers` allows to convert an ibc-originated asset to its erc20 representation and `transfer-tokens` allows to send a erc20 to another cosmos chain through ibc.
...
- mint_by_elysium_module(address user, uint256 amount)
- burn_by_elysium_module(address user, uint256 amount)
...
-  __ElysiumSendToAccount(address recipient, uint256 amount) : send a specific token amount from contract address to recipient address
-  __ElysiumSendToIBCChannel(address recipient, uint256 amount) : send a specific token amount through ibc to recipient address in destination chain
...
* https://github.com/furyaxyz/elysium/blob/main/contracts/src/ModuleFRC20.sol
* https://github.com/furyaxyz/elysium/blob/main/docs/architecture/adr-002.md