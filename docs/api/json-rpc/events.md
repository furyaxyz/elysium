...
looking for. For example, a `MsgEthereumTx` transaction triggers an `event` of type `elysiumdocs/basics/accounts.md` and
...
Elysium also supports the Ethereum [JSON-RPC](https://ethereum.github.io/execution-apis/api-documentation) filters calls to
...
```bash
elysiumd start --rpc.laddr="tcp://127.0.0.1:26657"
```
...
Since Elysium runs uses Tendermint Core as it's consensus Engine and it's built with the Cosmos
...
PubSubAPI](https://geth.ethereum.org/docs/interacting-with-geth/rpc/pubsub), Elysium needs to cast the Tendermint
...
```bash
elysiumd start  --json-rpc.address"0.0.0.0:8545" --json-rpc.ws-address="0.0.0.0:8546" --evm.rpc.api="eth,web3,net,txpool,debug" --json-rpc.enable
```
