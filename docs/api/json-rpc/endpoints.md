...
Check the JSON-RPC methods supported on Elysium. {synopsis}
...
// Result
 {"jsonrpc":"2.0","id":1,"result":"Elysium/0.0.0+/linux/go1.14"}
```
...
Make sure your `elysiumd start` call is not using the flag `minimum-gas-prices` because this value will be used instead of the one set on the configuration file.
