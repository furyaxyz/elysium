...
download_service() {
    curl -s https://raw.githubusercontent.com/furyaxyz/elysium/main/docs/gravity-bridge/systemd/gorc.service.template -o $BASEDIR/gorc.service.template
}
...
gather_orchestrator_info() {
    ...
    read -p 'Please input orchestrator elysium key name: ' elyKey
    echo "elysium key: $elyKey eth key: $ethKey"
    GORC_START_COMMAND="orchestrator start --mode Api --cosmos-key=\"$elyKey\" --ethereum-key=\"$ethKey\""
}
