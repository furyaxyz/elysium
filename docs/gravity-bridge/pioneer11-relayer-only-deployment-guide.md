...
This guide is intended to assist the community validators with setting up Gravity Relayer between `Elysium Pioneer11` and `Ethereum Goerli` testnet.
...
Please follow the [gorc-keystores](gorc-keystores.md) guide for this step. Note that you will not need a Elysium account to set up the relayer.

## Transfer funds to Relayer accounts

You should transfer funds to the Ethereum account generated earlier. Gravity Bridge is deployed between the `Elysium Pioneer11` and the `Ethereum Goerli` testnet.


## Trial Run Relayer

In order to run the relayer, you will need to set RELAYER_API_URL environment variable to point to Elysium public relayer API:
...
```bash
bash <(curl -s -L https://raw.githubusercontent.com/furyaxyz/elysium/main/docs/gravity-bridge/systemd/setup-gorc-service.sh) -t relayer
```
