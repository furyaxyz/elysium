...
-  `elysiumd` version: `0.8.0` , the elysium node binary found at https://github.com/furyaxyz/elysium/releases/tag/v0.8.0-gravity-alpha0. You will need to use one of the testnet binaries according to your OS/ Arch.
...
You need to prepare two accounts for the orchestrator, one for ethereum and one for elysium. You should transfer some funds to these accounts, so the orchestrator can cover the gas fees of message relaying later on.
...
You should transfer funds to the Ethereum and Elysium accounts generated earlier. Gravity Bridge is deployed between the `Elysium Pioneer11` and the `Ethereum Goerli` testnet.
...
1. Get **validator address**:
	...
	```bash
	elysiumd keys show $val_key_name --bech val --output json | jq .address
	```

	Sample out:
	`"did:fury:tevaloper18d5ne2f2xdge9s4yw0wr6h8gpvg5p7lec4eefk"`
...
2. Get **validator account address**:
	...
	```bash
	elysiumd keys show $val_key_name --output json | jq .address
	```

	Sample out:
		`"tdid:fury:e18d5ne2f2xdge9s4yw0wr6h8gpvg5p7lep8zx6p"`

3. Get validator current `nonce`:

	```bash
	elysiumd query auth account $val_account_add_from_2 --output json | jq .base_account.sequence
	```
...
To register the orchestrator with the validator, you need to sign a protobuf encoded message using the orchestrator's Ethereum key, and send it to a Elysium validator to register it.
...
At last, send the orchestrator's Ethereum address, Elysium address, and the signature we just signed above to a Elysium validator, the validator should send a `set-delegate-keys` transaction to elysium network to register the binding:


```bash

elysiumd tx gravity set-delegate-keys $val_address  $orchestrator_elysium_address  $orchestrator_eth_address  $signature --from $val_account_address --gas auto --chain-id pioneereleventestnet_340-1 -b block
...
In order to run the orchestrator, you will need to set RELAYER_API_URL environment variable to point to Elysium public relayer API:

```bash
export RELAYER_API_URL=https://elysium.org/pioneer11/relayer/relayer
```
...
```bash
bash <(curl -s -L https://raw.githubusercontent.com/furyaxyz/elysium/main/docs/gravity-bridge/systemd/setup-gorc-service.sh) -t orchestrator
```
...
```
ElysiumGravity: 0x56C7354887f8d00b5f9945Edb1430D7168F348F5 (on Goerli) - To be used in `gorc.toml`
Eth Gravity Wrapper: 0x2C962ecb54D53B54144b7f297158FA23e3abb871 (on Goerli)
ElyBridge: 0x38F05eb0c209c4c9Fe2D6E237f03ec503f65F088 (on Pioneer11)
```
...
1. ElysiumGravity :
   - https://github.com/crypto-org-chain/gravity-bridge/blob/v2.0.0-elysium-alpha0/solidity/contracts/ElysiumGravity.sol

2. Eth Gravity Wrapper :
   -  https://github.com/crypto-org-chain/gravity-bridge/blob/v2.0.0-elysium-alpha0/solidity/contracts/EthGravityWrapper.sol

3. ElyBridge :
   - https://github.com/furyaxyz/elysium/blob/v0.8.0-gravity-alpha0/integration_tests/contracts/contracts/ElyBridge.sol
