  ...
  outputs = { self, nixpkgs, nix-bundle-exe, gomod2nix, flake-utils }:
    ...
    (flake-utils.lib.eachDefaultSystem
      (system:
        ...
        in
        rec {
          packages = pkgs.elysium-matrix // {
            inherit (pkgs) rocksdb;
          };
          apps = {
            elysiumd = mkApp packages.elysiumd;
            elysiumd-testnet = mkApp packages.elysiumd-testnet;
          };
          defaultPackage = packages.elysiumd;
          defaultApp = apps.elysiumd;
          devShells = {
            elysiumd = pkgs.mkShell {
      ...
      overlay = final: super: {
        ...
        make-tarball = drv: final.runCommand "tarball-${drv.name}"
          ...
          tar cfv - -C "${drv}" \
            ...
            | gzip -9 > $out
        '';
        bundle-win-exe = drv: final.callPackage ./nix/bundle-win-exe.nix { elysiumd = drv; };
      } // (with final;
        let
          ...
          binaries = builtins.listToAttrs (builtins.map
            ({ network, pkgtype }: {
              ...
              value =
                let
                  elysiumd = callPackage ./. {
                    inherit rev network;
                  };
                  bundle =
                    if stdenv.hostPlatform.isWindows then
                      bundle-win-exe elysiumd
                    else
                      bundle-exe elysiumd;
                in
