...
ldflags += -X github.com/cosmos/cosmos-sdk/version.Name=elysium \
	-X github.com/cosmos/cosmos-sdk/version.AppName=elysiumd \
...
build: check-network print-ledger go.sum
	@go build -mod=readonly $(BUILD_FLAGS) -o $(BUILDDIR)/elysiumd ./cmd/elysiumd

install: check-network print-ledger go.sum
	@go install -mod=readonly $(BUILD_FLAGS) ./cmd/elysiumd
...
SIMAPP = github.com/furyaxyz/elysium/v2/app
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.elysiumd/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.elysiumd/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
test-elysium-contracts:
	@git submodule update --init --recursive
	@nix-shell ./contracts/shell.nix --pure --run ./scripts/test-elysium-contracts

gen-elysium-contracts:
	@git submodule update --init --recursive
	@nix-shell ./contracts/shell.nix --pure --run ./scripts/gen-elysium-contracts

.PHONY: gen-elysium-contracts test-elysium-contracts
...
HTTPS_GIT := https://github.com/furyaxyz/elysium.git
