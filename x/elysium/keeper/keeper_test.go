...
func (suite *KeeperTestSuite) RegisterSourceToken(
	...
	return suite.app.ElysiumKeeper.RegisterOrUpdateTokenMapping(suite.ctx, &msg)
}
...
func (suite *KeeperTestSuite) TestOnRecvVouchers() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			sdk.NewCoins(sdk.NewCoin(types.IbcElyDenomDefaultValue, sdk.NewInt(123)), sdk.NewCoin("bad", sdk.NewInt(10))),
			func() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			tc.malleate()
			suite.app.ElysiumKeeper.OnRecvVouchers(suite.ctx, tc.coins, address.String())
			tc.postCheck()
		})
	}
}
...
func (suite *KeeperTestSuite) TestRegisterOrUpdateTokenMapping() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				err := suite.app.ElysiumKeeper.SetExternalContractForDenom(
					suite.ctx,
		...
		},
		{
			...
			types.MsgUpdateTokenMapping{
				Sender:   "",
				Denom:    "elysium0xf6d4fecb1a6fb7c2ca350169a050d483bd87b88@",
				Contract: contractAddress,
				Symbol:   "",
				Decimal:  0,
			},
		...
		},
		{
			...
			types.MsgUpdateTokenMapping{
				Sender:   "",
				Denom:    "elysium0xF6D4FeCB1a6fb7C2CA350169A050D483bd87b883",
				Contract: contractAddress,
				Symbol:   "",
				Decimal:  0,
			},
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			tc.malleate()
			err := suite.app.ElysiumKeeper.RegisterOrUpdateTokenMapping(suite.ctx, &tc.msg)
			if tc.error {
				suite.Require().Error(err)
			} else {
				suite.Require().NoError(err)
