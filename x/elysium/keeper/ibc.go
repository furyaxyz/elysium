...
func (k Keeper) ConvertVouchersToEvmCoins(ctx sdk.Context, from string, coins sdk.Coins) error {
	...
	for _, c := range coins {
		switch c.Denom {
		case params.IbcElyDenom:
			if params.IbcElyDenom == "" {
				return errorsmod.Wrap(types.ErrIbcElyDenomEmpty, "ibc is disabled")
			}
			...
			// Compute new amount, because baseely is a 8 decimals token, we need to multiply by 10^10 to make it
			// a 18 decimals token
		...
		default:
			err := k.ConvertCoinFromNativeToFRC21(ctx, common.BytesToAddress(acc.Bytes()), c, params.EnableAutoDeployment)
			if err != nil {
				return err
			}
		}
...
func (k Keeper) IbcTransferCoins(ctx sdk.Context, from, destination string, coins sdk.Coins, channelId string) error {
	...
	for _, c := range coins {
		...
		case evmParams.EvmDenom:
			...
			ibcCoin := sdk.NewCoin(params.IbcElyDenom, amount8dec)
			if err := k.bankKeeper.SendCoinsFromModuleToAccount(
		...
		default:
			if !types.IsValidIBCDenom(c.Denom) && !types.IsValidElysiumDenom(c.Denom) {
				return fmt.Errorf("the coin %s is neither an ibc voucher or a elysium token", c.Denom)
			}
