...
func (k Keeper) doAfterSendToCosmosEvent(ctx sdk.Context, event gravitytypes.SendToCosmosEvent) error {
	...
	ctx.EventManager().EmitEvent(sdk.NewEvent(
		elysiumtypes.EventTypeEthereumSendToCosmosHandled,
		sdk.NewAttribute(sdk.AttributeKeyModule, gravitytypes.ModuleName),
		sdk.NewAttribute(elysiumtypes.AttributeKeySender, event.GetEthereumSender()),
		sdk.NewAttribute(elysiumtypes.AttributeKeyReceiver, event.GetCosmosReceiver()),
		sdk.NewAttribute(elysiumtypes.AttributeKeyAmount, coins.String()),
		sdk.NewAttribute(gravitytypes.AttributeKeyBridgeChainID, strconv.FormatUint(
			k.gravityKeeper.GetParams(ctx).BridgeChainId, 10,
		)),
		sdk.NewAttribute(elysiumtypes.AttributeKeyEthereumTokenContract, event.GetTokenContract()),
	...
	err = k.ConvertCoinFromNativeToFRC21(ctx, addr, coin, enableAutoDeployment)
	if err != nil {
		return err
	}
