...
// `event __ElysiumSendToIbc(address indexed sender, string indexed recipient, string indexed channel_id, uint256 amount, bytes extraData)`
...
func NewSendToIbcV2Handler(bankKeeper types.BankKeeper, elysiumKeeper elysiumkeeper.Keeper) *SendToIbcV2Handler {
	return &SendToIbcV2Handler{
		SendToIbcHandler: NewSendToIbcHandler(bankKeeper, elysiumKeeper),
	}
}
