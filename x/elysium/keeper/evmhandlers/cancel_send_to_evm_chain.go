...
// `event __ElysiumCancelSendToEvmChain(uint256 id)`
...
func (h CancelSendToEvmChainHandler) Handle(
	...
	if len(topics) != 2 {
		// log and ignore
		h.elysiumKeeper.Logger(ctx).Info("log signature matches but wrong number of indexed events")
		for i, topic := range topics {
			h.elysiumKeeper.Logger(ctx).Debug(fmt.Sprintf("topic index: %d value: %s", i, topic.TerminalString()))
		}
		return nil
	}
	...
	frc20Address, found := h.elysiumKeeper.GetContractByDenom(ctx, denom)
	if !found {
		return fmt.Errorf("the native token %s is not associated with any contract address on elysium", denom)
	}
	if frc20Address != contract {
	...
	err = h.elysiumKeeper.ConvertVouchersToEvmCoins(ctx, senderCosmosAddr.String(), refundAmount)
	if err != nil {
		return err
	}
	return nil
