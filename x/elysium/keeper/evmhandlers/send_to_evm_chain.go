...
func (h SendToEvmChainHandler) Handle(
	...
	if !types.IsValidGravityDenom(denom) && !types.IsValidElysiumDenom(denom) {
		return fmt.Errorf("the native token associated with the contract %s is neither a gravity voucher or a elysium token", contract)
	}
