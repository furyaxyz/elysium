...
func (h SendToIbcHandler) handle(
	...
	if !types.IsValidIBCDenom(denom) && !types.IsValidElysiumDenom(denom) {
		return fmt.Errorf("the native token associated with the contract %s is neither an ibc voucher or a elysium token", contract)
	}
	...
	if err = h.elysiumKeeper.IbcTransferCoins(ctx, sender.String(), recipient, coins, channelId); err != nil {
		return err
	}
	return nil
}
