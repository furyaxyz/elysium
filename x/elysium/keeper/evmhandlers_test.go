...
import (
	...
	evmhandlers "github.com/furyaxyz/elysium/v2/x/elysium/keeper/evmhandlers"
...
func (suite *KeeperTestSuite) TestSendToAccountHandler() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {
				suite.app.ElysiumKeeper.SetExternalContractForDenom(suite.ctx, denom, contract)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			handler := evmhandlers.NewSendToAccountHandler(suite.app.BankKeeper, suite.app.ElysiumKeeper)
			tc.malleate()
...
func (suite *KeeperTestSuite) TestSendToEvmChainHandler() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				suite.app.ElysiumKeeper.SetExternalContractForDenom(suite.ctx, invalidDenom, contract)
			...
			func() {},
			errors.New("the native token associated with the contract 0x0000000000000000000000000000000000000001 is neither a gravity voucher or a elysium token"),
		},
		{
			"non supported network id",
			func() {
				suite.app.ElysiumKeeper.SetExternalContractForDenom(suite.ctx, validDenom, contract)
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToEvmChainHandler(
				...
				suite.app.BankKeeper, suite.app.ElysiumKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendToIbcHandler() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {},
			errors.New("the native token associated with the contract 0x0000000000000000000000000000000000000001 is neither an ibc voucher or a elysium token"),
		},
		{
			"success send to ibc",
			func() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToIbcHandler(suite.app.BankKeeper, elysiumKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendToIbcV2Handler() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToIbcV2Handler(suite.app.BankKeeper, elysiumKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendElyToIbcHandler() {
	contract := common.BigToAddress(big.NewInt(1))
	sender := common.BigToAddress(big.NewInt(2))
	var data []byte
	var topics []common.Hash
	...
	testCases := []struct {
	...
	}{
		{
			...
			func() {
				...
				topics = []common.Hash{
					evmhandlers.SendElyToIbcEvent.ID,
				}
				input, err := evmhandlers.SendElyToIbcEvent.Inputs.NonIndexed().Pack(
					sender,
					"recipient",
					coin.Amount.BigInt(),
				)
		...
		},
		{
			...
			func() {
				...
				suite.MintCoinsToModule(types.ModuleName, sdk.NewCoins(sdk.NewCoin(types.IbcElyDenomDefaultValue, sdk.NewInt(123))))
				topics = []common.Hash{
					evmhandlers.SendElyToIbcEvent.ID,
				}
				input, _ := evmhandlers.SendToIbcEvent.Inputs.NonIndexed().Pack(
			...
			func() {
				...
				coin := sdk.NewCoin(types.IbcElyDenomDefaultValue, sdk.NewInt(0))
				balance := suite.app.BankKeeper.GetBalance(suite.ctx, sdk.AccAddress(types.ModuleName), types.IbcElyDenomDefaultValue)
				suite.Require().Equal(coin, balance)
				ibcCoin := sdk.NewCoin(types.IbcElyDenomDefaultValue, sdk.NewInt(123))
				// As we mock IBC module, we expect the token to be in user balance
				ibcBalance := suite.app.BankKeeper.GetBalance(suite.ctx, sdk.AccAddress(sender.Bytes()), types.IbcElyDenomDefaultValue)
				suite.Require().Equal(ibcCoin, ibcBalance)
				elyCoin := sdk.NewCoin(suite.evmParam.EvmDenom, sdk.NewInt(500))
				elyBalance := suite.app.BankKeeper.GetBalance(suite.ctx, sdk.AccAddress(sender.Bytes()), suite.evmParam.EvmDenom)
				suite.Require().Equal(elyCoin, elyBalance)
			},
			nil,
		},
	}
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendElyToIbcHandler(suite.app.BankKeeper, elysiumKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewCancelSendToEvmChainHandler(
				gravitykeeper.NewMsgServerImpl(suite.app.GravityKeeper),
				suite.app.ElysiumKeeper, suite.app.GravityKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
