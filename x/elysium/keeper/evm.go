...
// CallModuleFRC21 call a method of ModuleFRC21 contract
func (k Keeper) CallModuleFRC21(ctx sdk.Context, contract common.Address, method string, args ...interface{}) ([]byte, error) {
	data, err := types.ModuleFRC21Contract.ABI.Pack(method, args...)
	if err != nil {
		return nil, err
	}
...
// DeployModuleFRC21 deploy an embed frc21 contract
func (k Keeper) DeployModuleFRC21(ctx sdk.Context, denom string) (common.Address, error) {
	ctor, err := types.ModuleFRC21Contract.ABI.Pack("", denom, uint8(0), false)
	if err != nil {
		return common.Address{}, err
	}
	data := types.ModuleFRC21Contract.Bin
...
// ConvertCoinFromNativeToFRC21 convert native token to erc20 token
func (k Keeper) ConvertCoinFromNativeToFRC21(ctx sdk.Context, sender common.Address, coin sdk.Coin, autoDeploy bool) error {
	...
	if !found {
		...
		contract, err = k.DeployModuleFRC21(ctx, coin.Denom)
		if err != nil {
			return err
		}
	...
	if isSource {
		...
		// unlock frc tokens
		_, err = k.CallModuleFRC21(ctx, contract, "transfer_from_elysium_module", sender, coin.Amount.BigInt())
		if err != nil {
			return err
		}
	} else {
		...
		// mint frc tokens
		_, err = k.CallModuleFRC21(ctx, contract, "mint_by_elysium_module", sender, coin.Amount.BigInt())
		if err != nil {
			return err
		}
	}
...
// ConvertCoinFromFRC21ToNative convert erc20 token to native token
func (k Keeper) ConvertCoinFromFRC21ToNative(ctx sdk.Context, contract common.Address, receiver common.Address, amount sdkmath.Int) error {
	...
	if isSource {
		_, err := k.CallModuleFRC21(ctx, contract, "transfer_by_elysium_module", receiver, amount.BigInt())
		if err != nil {
			return err
		}
		...
		_, err = k.CallModuleFRC21(ctx, contract, "burn_by_elysium_module", receiver, amount.BigInt())
		if err != nil {
			return err
		}
	}
...
// ConvertCoinsFromNativeToFRC21 convert native tokens to erc20 tokens
func (k Keeper) ConvertCoinsFromNativeToFRC21(ctx sdk.Context, sender common.Address, coins sdk.Coins, autoDeploy bool) error {
	for _, coin := range coins {
		if err := k.ConvertCoinFromNativeToFRC21(ctx, sender, coin, autoDeploy); err != nil {
			return err
		}
	}
	return nil
