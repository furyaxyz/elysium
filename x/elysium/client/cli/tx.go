...
func CmdConvertTokens() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Convert ibc vouchers to elysium tokens, Note, the'--from' flag is" +
...
func CmdSendToCryptoOrg() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Transfer elysium tokens to the origin chain through IBC , Note, the'--from' flag is" +
...
// CmdUpdatePermissions returns a CLI command handler for updating elysium permissions
