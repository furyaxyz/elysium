...
func init() {
	if err := evmrpc.RegisterAPINamespace(ElysiumNamespace, CreateElysiumRPCAPIs); err != nil {
		panic(err)
	}
}
...
func CreateElysiumRPCAPIs(ctx *server.Context, clientCtx client.Context, tmWSClient *rpcclient.WSClient, allowUnprotectedTxs bool, indexer ethermint.EVMTxIndexer) []rpc.API {
	...
	return []rpc.API{
		{
			Namespace: ElysiumNamespace,
			Version:   apiVersion,
			Service:   NewElysiumAPI(ctx.Logger, clientCtx, *evmBackend),
			Public:    true,
		},
	}
}
...
type ElysiumAPI struct {
	...
	elysiumQueryClient types.QueryClient
}
...
func NewElysiumAPI(
	...
	return &ElysiumAPI{
		...
		backend:           backend,
		elysiumQueryClient: types.NewQueryClient(clientCtx),
	}
}

func (api *ElysiumAPI) getBlockDetail(blockNrOrHash rpctypes.BlockNumberOrHash) (
...
func (api *ElysiumAPI) GetTransactionReceiptsByBlock(blockNrOrHash rpctypes.BlockNumberOrHash) ([]map[string]interface{}, error) {
	api.logger.Debug("elysium_getTransactionReceiptsByBlock", "blockNrOrHash", blockNrOrHash)
...
func (api *ElysiumAPI) ReplayBlock(blockNrOrHash rpctypes.BlockNumberOrHash, postUpgrade bool) ([]map[string]interface{}, error) {
	api.logger.Debug("elysium_replayBlock", "blockNrOrHash", blockNrOrHash)
	resBlock, blockNumber, blockHash, blockRes, baseFee, err := api.getBlockDetail(blockNrOrHash)
	if err != nil {
		return nil, err
	}
	...
	rsp, err := api.elysiumQueryClient.ReplayBlock(rpctypes.ContextWithHeight(contextHeight), req)
	if err != nil {
		return nil, err
	}
...
func (api *ElysiumAPI) getBlock(blockNrOrHash rpctypes.BlockNumberOrHash) (blk *coretypes.ResultBlock, err error) {
