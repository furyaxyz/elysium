...
func NewIBCConversionModule(app porttypes.IBCModule, ck elysiumkeeper.Keeper) IBCConversionModule {
...
func (im IBCConversionModule) convertVouchers(ctx sdk.Context, data transferTypes.FungibleTokenPacketData, denom string, isSender bool) error {
	...
	if isSender {
		im.elysiumkeeper.OnRecvVouchers(ctx, sdk.NewCoins(token), data.Sender)
	} else {
		im.elysiumkeeper.OnRecvVouchers(ctx, sdk.NewCoins(token), data.Receiver)
	}
	return nil
}

func (im IBCConversionModule) canBeConverted(ctx sdk.Context, denom string) bool {
	params := im.elysiumkeeper.GetParams(ctx)
	if denom == params.IbcElyDenom {
		return true
	}
	_, found := im.elysiumkeeper.GetContractByDenom(ctx, denom)
	return found
}
